...
class RegistroU(Top):
    ...
    def registrationFrame(self):
        ...
        self.botonMasculino = Radiobutton(self.frameRegist, variable=self.generoSeleccionado, value="M", text="Masculino",bg="#fff",highlightthickness=0, relief=FLAT, cursor="hand2" ,font=('TKDefaultFont', 11))
        self.botonFemenino = Radiobutton(self.frameRegist, variable=self.generoSeleccionado, value="F", text="Femenino",bg="#fff",highlightthickness=0, relief=FLAT, cursor="hand2" ,font=('TKDefaultFont', 11))
        ...
        Button(self.frameRegist, width=23, pady=7, text='Enviar', bg='#57a1f8', fg='white', border=0, cursor="hand2", command=self.comprobar).place(x=100, y=298)

    def comprobar(self):
             
        self.nombreDado = self.nombreEntry.get()
             
        if self.nombreDado != "Ingresar Nombre " and self.nombreDado != "":
            print(f"El nombre no esta vacio {(self.nombreDado).strip()}")
        else:
